...
.names <- purrr::list_modify(.names_plume, !!!.names_quarto)

make_table_vars <- function(category) {
  vars_plume <- fetch(.names_plume, category)
  vars_plume_quarto <- fetch(.names_quarto, category)
  vars <- fetch(.names, category)
  tibble::tibble(
    name = list_keys(vars),
    Plume = ifelse(unlist(vars) %in% unlist(vars_plume), 1, 0),
    PlumeQuarto = ifelse(unlist(vars) %in% unlist(vars_plume_quarto), 1, 0),
  ) |>
    gt() |>
    text_case_match(
...
plume uses `r plume:::link("crt")` (CRediT) by default (assuming the input data contains the [appropriate columns](#constants)). You can specify your own roles via the `roles` parameter when creating a `plume` object. The `roles` parameter takes a vector of key-value pairs where keys define role columns and values the actual roles to use.
...
set_methods <- tibble::tibble(
  ...
  Plume = c(1, 1, 0, 0),
  PlumeQuarto = c(1, 0, 1, 1)
)
```

plume provides `r nrow(set_methods)` methods to set particular status to authors:
...
By default, `set_*()` methods assign values by authors' id. You can change this behaviour at the object (in `PlumeQuarto` only) or method level using the `by`/`.by` parameter.
