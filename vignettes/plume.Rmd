...
```{r, echo = FALSE}
make_table_vars <- function(category, protected = FALSE) {
  vars <- list(
    Plume = plume:::default_names,
    PlumeQuarto = plume:::default_names_quarto,
    protected = plume:::.names$protected
  )
  type <- if (protected) "protected" else "PlumeQuarto"
  tibble::tibble(
    name = names(vars[[type]][[category]]),
    Plume = ifelse(name %in% unlist(vars$Plume) | protected, 1, 0),
    PlumeQuarto = 1
  ) |>
    gt() |>
    text_case_match(
...
**Constants**: these names are standardised and must be provided as is.

```{r, echo = FALSE}
make_table_vars("crt", protected = TRUE)
```
...
```{r}
#| eval: false
aut$get_orcids(compact = TRUE, icon = FALSE, sep = ": ")
```
...
#| echo: false
#| results: asis
cat(aut$get_orcids(compact = TRUE, icon = FALSE, sep = ": "), sep = "\n\n")
```
:::
:::
