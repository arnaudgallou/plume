...
It's possible to force one or more contributors to appear first in any given role using the `set_main_contributors()` method:
...
You can easily assign the same main contributors across multiple roles using the `.roles` parameter:

```{r}
aut$set_main_contributors(jean, .roles = c("supervision", "writing"), .by = "given_name")
aut$get_contributions()
```

Note that main contributors have the priority over the `alphabetical_order` parameter:
